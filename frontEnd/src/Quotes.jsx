  ...
  return (
    <section className="quotes-page">
      ...
      {selectedQuote && <section className="quotes-panel">
        ...
        {isItemsLoading ? <p className="empty-state">Chargement des lignes...</p> : quoteItems.length === 0 ? <p className="empty-state">Aucune ligne pour ce devis.</p> : <div className="quotes-table"><div className="quote-row quote-row-head"><span>DESCRIPTION</span><span>PRODUIT</span><span>QTE</span><span>PRIX UNITAIRE</span><span>TVA</span><span>TOTAL</span><span>ACTIONS</span></div>{quoteItems.map((item) => <div className="quote-row" key={item.id}><strong>{item.description}</strong><span>{item.product ? item.product.name : '-'}</span><span>{item.quantity}</span><span>{formatPrice(item.unit_price)}</span><span>{item.tax_rate ? `${item.tax_rate} %` : '-'}</span><strong>{formatPrice(item.total)}</strong><div className="quote-actions"><button type="button" onClick={() => startEditItem(item)}>Modifier</button><button className="delete-button" type="button" onClick={() => handleItemDelete(item)}>Supprimer</button></div></div>)}</div>}
      </section>}
      ...
      <section className="quotes-panel">
        ...
        {isLoading ? <p className="empty-state">Chargement des devis...</p> : quotes.length === 0 ? <p className="empty-state">Aucun devis pour le moment. Créez votre premier devis.</p> : <div className="quotes-table"><div className="quote-row quote-row-head"><span>N DEVIS</span><span>CLIENT</span><span>DATE</span><span>VALIDE JUSQU AU</span><span>MONTANT</span><span>STATUT</span><span>ACTIONS</span></div>{quotes.map((quote) => <div className="quote-row" key={quote.id}><strong>{quote.quote_number}</strong><span>{quote.client ? quote.client.name : '-'}</span><span>{formatDate(quote.quote_date)}</span><span>{formatDate(quote.valid_until)}</span><strong>{formatPrice(quote.total_amount)}</strong><select className={`status-select status-${quote.status}`} value={quote.status} onChange={(event) => handleStatusChange(quote, event.target.value)}>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><div className="quote-actions"><button type="button" onClick={() => openEditForm(quote)}>Modifier</button><button type="button" onClick={() => openItemsManager(quote)}>Lignes</button>{quote.status !== 'converted' && <button className="convert-button" type="button" onClick={() => handleConvertToInvoice(quote)}>Convertir</button>}<button className="delete-button" type="button" onClick={() => handleDelete(quote)}>Supprimer</button></div></div>)}</div>}
      </section>
    </section>
  )
}
